...
export default function RolesMatrixPage() {
    ...
    return <main className="mx-auto max-w-7xl space-y-7 px-6 py-8">
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
            ...
            <form onSubmit={addNode} className="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm"><div className="flex items-center gap-2"><Plus size={18} className="text-emerald-700" /><h2 className="font-black text-neutral-900">Add workflow node</h2></div><p className="mt-1 text-xs text-neutral-500">Use a permission as the source of truth for a group action.</p>{!canManageWorkflows && <p className="mt-4 rounded-lg bg-amber-50 p-3 text-xs font-semibold text-amber-800">You need WORKFLOW_MANAGE permission to change this group&apos;s workflow.</p>}<div className="mt-5 space-y-3">{[['actionKey', 'Action key'], ['label', 'Label'], ['requiredPermission', 'Required permission'], ['stepOrder', 'Step order']].map(([key, label]) => <label key={key} className="block text-xs font-bold text-neutral-600">{label}<Input disabled={!canManageWorkflows} value={(form as any)[key]} onChange={(event) => setForm({ ...form, [key]: event.target.value })} className="mt-1 w-full rounded-lg border border-neutral-200 px-3 py-2.5 text-sm outline-none focus:border-emerald-600 disabled:bg-neutral-100" /></label>)}<label className="block text-xs font-bold text-neutral-600">Required role<NativeSelect disabled={!canManageWorkflows} value={form.requiredRole} onChange={(event) => setForm({ ...form, requiredRole: event.target.value })} className="mt-1 w-full rounded-lg border border-neutral-200 px-3 py-2.5 text-sm disabled:bg-neutral-100"><option>GROUP_ADMIN</option><option>GROUP_CHAIRMAN</option><option>ACCOUNTANT</option><option>TREASURER</option><option>MEMBER</option></NativeSelect></label><Button type="submit" disabled={loading || !groupId || !canManageWorkflows} className="flex w-full items-center justify-center gap-2 rounded-lg bg-emerald-700 px-4 py-3 text-sm font-black text-white disabled:opacity-50">{loading && <Loader2 size={16} className="animate-spin" />} Save workflow node</Button></div></form>
        </section>
    </main>
}
