...
export default function PaymentsPage() {
    ...
    return <main className="min-h-screen bg-linear-to-br from-neutral-50 to-emerald-50/30 px-4 py-8 sm:px-6"><div className="mx-auto max-w-7xl space-y-7">
        ...
        <section className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="relative flex-1"><Search size={16} className="absolute left-3 top-3 text-neutral-400" /><Input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search member or reference" className="w-full rounded-lg border border-neutral-200 py-2.5 pl-10 pr-3 text-sm outline-none focus:border-emerald-500" /></div><div className="flex gap-2"><NativeSelect value={typeFilter} onChange={event => setTypeFilter(event.target.value)} className="rounded-lg border border-neutral-200 px-3 py-2.5 text-xs font-semibold"><option value="ALL">All allocations</option><option value="CONTRIBUTION">Contributions</option><option value="SHARE_PURCHASE">Share purchases</option><option value="LOAN_REPAYMENT">Loan repayments</option><option value="FINE">Fines</option><option value="SOCIAL_FUND">Social fund</option><option value="OTHER">Other</option></NativeSelect><NativeSelect value={methodFilter} onChange={event => setMethodFilter(event.target.value)} className="rounded-lg border border-neutral-200 px-3 py-2.5 text-xs font-semibold"><option value="ALL">All methods</option><option value="CASH">Cash</option><option value="BANK">Bank</option><option value="MOBILE_MONEY">Mobile money</option><option value="CONTROL_NUMBER">Control number</option></NativeSelect></div></div></section>
        <section className="overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm"><div className="flex items-center gap-2 border-b border-neutral-100 p-5"><WalletCards size={18} className="text-emerald-600" /><div><h2 className="font-black text-neutral-900">All payment activity</h2><p className="text-xs text-neutral-500">Live records from the group payment table</p></div></div><div className="overflow-x-auto"><Table className="w-full text-left text-sm"><TableHeader className="bg-neutral-50 text-xs uppercase tracking-wider text-neutral-500"><TableRow><TableHead className="px-5 py-3">Reference</TableHead><TableHead className="px-5 py-3">Member</TableHead><TableHead className="px-5 py-3">Allocation</TableHead><TableHead className="px-5 py-3">Method</TableHead><TableHead className="px-5 py-3">Date</TableHead><TableHead className="px-5 py-3 text-right">Amount</TableHead><TableHead className="px-5 py-3">Status</TableHead></TableRow></TableHeader><TableBody className="divide-y divide-neutral-100">{filtered.map(payment => <TableRow key={payment.id} className="hover:bg-neutral-50"><TableCell className="px-5 py-4 font-bold text-emerald-700">{payment.reference}</TableCell><TableCell className="px-5 py-4 font-bold text-neutral-800">{payment.memberName}<span className="block text-xs font-normal text-neutral-400">{payment.membershipNumber || "Group payment"}</span></TableCell><TableCell className="px-5 py-4"><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">{(payment.allocationType || "OTHER").replaceAll("_", " ")}</span></TableCell><TableCell className="px-5 py-4 text-xs text-neutral-600">{payment.paymentMethod.replaceAll("_", " ")}</TableCell><TableCell className="px-5 py-4 text-xs text-neutral-500">{new Date(payment.paymentDate).toLocaleDateString()}</TableCell><TableCell className="px-5 py-4 text-right font-black">{money(payment.amount)}</TableCell><TableCell className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${payment.status === "COMPLETED" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{payment.status}</span></TableCell></TableRow>)}{filtered.length === 0 && <TableRow><TableCell colSpan={7} className="px-5 py-14 text-center text-neutral-500">No payments found for this group.</TableCell></TableRow>}</TableBody></Table></div></section>
        {showForm && <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-950/40 p-4"><div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-xl font-black">Record payment</h2><p className="text-xs text-neutral-500">Allocate one payment to the correct module.</p></div><Button onClick={() => setShowForm(false)} className="text-sm font-bold text-neutral-400">Close</Button></div><form onSubmit={submit} className="space-y-4"><NativeSelect value={form.memberId} onChange={event => setForm({ ...form, memberId: event.target.value })} className="w-full rounded-lg border border-neutral-200 px-3 py-3 text-sm"><option value="">Group payment / no member</option>{members.map(member => <option key={member.id} value={String(member.id)}>{member.name || member.fullName || member.memberNo}</option>)}</NativeSelect><Input required type="number" min="1" value={form.amount} onChange={event => setForm({ ...form, amount: event.target.value })} placeholder="Amount" className="w-full rounded-lg border border-neutral-200 px-3 py-3 text-sm" /><div className="grid grid-cols-2 gap-3"><NativeSelect value={form.type} onChange={event => setForm({ ...form, type: event.target.value })} className="rounded-lg border border-neutral-200 px-3 py-3 text-sm"><option value="OTHER">Other</option><option value="CONTRIBUTION">Contribution</option><option value="SHARE_PURCHASE">Share purchase</option><option value="LOAN_REPAYMENT">Loan repayment</option><option value="FINE">Fine</option><option value="SOCIAL_FUND">Social fund</option></NativeSelect><NativeSelect value={form.method} onChange={event => setForm({ ...form, method: event.target.value })} className="rounded-lg border border-neutral-200 px-3 py-3 text-sm"><option value="CASH">Cash</option><option value="BANK">Bank</option><option value="MOBILE_MONEY">Mobile money</option><option value="CONTROL_NUMBER">Control number</option></NativeSelect></div><Input value={form.reference} onChange={event => setForm({ ...form, reference: event.target.value })} placeholder="Reference (optional)" className="w-full rounded-lg border border-neutral-200 px-3 py-3 text-sm" /><Textarea value={form.description} onChange={event => setForm({ ...form, description: event.target.value })} placeholder="Description" className="w-full rounded-lg border border-neutral-200 px-3 py-3 text-sm" rows={3} /><Button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 py-3 text-sm font-bold text-white disabled:opacity-50">{loading && <Loader2 size={16} className="animate-spin" />} Save payment</Button></form></div></div>}
    </div></main>;
}
