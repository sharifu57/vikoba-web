...
export default function JamiiFundPage() {
  ...
  const [form, setForm] = useState({ memberId: "", fundTypeId: "", amount: "", reason: "" });
  ...
  const memberLabel = (member: Member) => member.name || member.fullName || [member.firstName, member.lastName].filter(Boolean).join(" ") || member.memberNo || "Member";
  const refresh = async () => {
    if (!groupId) return;
    try {
      const [requestData, summaryData, typeData, memberData] = await Promise.all([api.requests(groupId), api.summary(groupId), api.types(groupId), memberService.list(groupId)]);
      setRequests(requestData || []); setSummary(summaryData || emptySummary); setTypes(typeData || []); setMembers(unwrap(memberData) as Member[]);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      await api.request(groupId, { groupMemberId: Number(form.memberId), fundTypeId: Number(form.fundTypeId), requestedAmount: Number(form.amount), reason: form.reason.trim() });
      setRequestOpen(false); setForm({ memberId: "", fundTypeId: "", amount: "", reason: "" }); setMessage("Jamii request submitted for approval."); await refresh();
  ...
  return <main className="min-h-full bg-background text-foreground">
      ...
      <Card className="overflow-hidden rounded-2xl shadow-sm">
        ...
        <CardContent className="p-0">{initialLoading ? <div className="flex min-h-64 items-center justify-center gap-2 text-sm text-muted-foreground"><Loader2 className="size-5 animate-spin text-primary" />Loading Jamii requests…</div> : <Table className="min-w-[920px]"><TableHeader><TableRow><TableHead>Member</TableHead><TableHead>Support type</TableHead><TableHead>Reference</TableHead><TableHead className="text-right">Requested</TableHead><TableHead className="text-right">Approved</TableHead><TableHead>Date</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>
          {visibleRequests.map((request) => <TableRow key={request.id}><TableCell><p className="font-semibold">{request.memberName}</p><p className="mt-0.5 text-xs text-muted-foreground">{request.membershipNumber || "Group member"}</p></TableCell><TableCell><p className="font-medium">{request.fundTypeName}</p>{request.reason && <p className="mt-0.5 max-w-52 truncate text-xs text-muted-foreground">{request.reason}</p>}</TableCell><TableCell className="font-mono text-xs text-muted-foreground">{request.reference}</TableCell><TableCell className="text-right font-semibold">{money(request.requestedAmount)}</TableCell><TableCell className="text-right font-semibold">{request.approvedAmount ? money(request.approvedAmount) : "—"}</TableCell><TableCell className="text-muted-foreground">{request.requestedDate}</TableCell><TableCell><Badge variant="outline" className={statusStyle[request.status] || "bg-muted text-muted-foreground"}>{request.status}</Badge></TableCell><TableCell><div className="flex justify-end gap-2">
            {request.status === "PENDING" && <><Button size="sm" onClick={() => { setApprovalRequest(request); setApprovedAmount(String(request.requestedAmount)); }} disabled={actingId !== null}>Approve</Button><Button size="sm" variant="destructive" onClick={() => void act(request.id, () => api.reject(groupId, request.id), "Request rejected.")} disabled={actingId !== null}>{actingId === request.id && <Loader2 className="animate-spin" />}Reject</Button></>}
            {request.status === "APPROVED" && <Button size="sm" onClick={() => void act(request.id, () => api.pay(groupId, request.id), "Support marked as paid.")} disabled={actingId !== null}>{actingId === request.id ? <Loader2 className="animate-spin" /> : <ArrowUpRight />}Disburse</Button>}
    ...
    <Dialog open={requestOpen} onOpenChange={(next) => { if (!api.loading) setRequestOpen(next); }}><DialogContent className="max-h-[90vh] overflow-y-auto border-border bg-card text-card-foreground sm:max-w-lg"><DialogHeader><DialogTitle className="text-xl font-black">Request Jamii support</DialogTitle><DialogDescription>Record the member, support category, amount, and reason for review.</DialogDescription></DialogHeader><form onSubmit={submit} className="space-y-5">
      <div className="space-y-2"><Label htmlFor="jamii-member">Member</Label><NativeSelect id="jamii-member" required value={form.memberId} onChange={(event) => setForm({ ...form, memberId: event.target.value })}><option value="">Select member</option>{members.map((member) => <option key={member.id} value={String(member.id)}>{memberLabel(member)}</option>)}</NativeSelect></div>
      ...
      <DialogFooter className="border-border bg-muted/50"><Button type="button" variant="outline" onClick={() => setRequestOpen(false)} disabled={api.loading}>Cancel</Button><Button type="submit" disabled={api.loading || !types.length || !form.memberId || !form.fundTypeId || Number(form.amount) <= 0 || !form.reason.trim()}>{api.loading && <Loader2 className="animate-spin" />}Submit for approval</Button></DialogFooter>
