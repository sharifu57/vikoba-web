...
export default function LoanDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
    ...
    {loading && !loan ? <Card><CardContent className="flex items-center gap-2 p-8"><Loader2 className="size-4 animate-spin" /> Loading loan…</CardContent></Card> : !loan ? <Card><CardContent className="p-8">Loan not found.</CardContent></Card> : <>
      ...
      <Card><CardHeader><CardTitle className="text-lg">Repayment schedule</CardTitle><p className="text-sm text-muted-foreground">Each missed installment attracts {money(loan.latePaymentFine || 0, currency)} after its due date. The applied fine becomes part of the outstanding balance.</p></CardHeader><CardContent>{rows.length ? <><div className="mb-4 grid gap-3 sm:grid-cols-3"><div className="rounded-lg bg-muted/50 p-3"><p className="text-xs text-muted-foreground">Monthly installments</p><p className="mt-1 font-bold">{rows.length}</p></div><div className="rounded-lg bg-muted/50 p-3"><p className="text-xs text-muted-foreground">Fine per missed installment</p><p className="mt-1 font-bold text-amber-700">{money(loan.latePaymentFine || 0, currency)}</p></div><div className="rounded-lg bg-muted/50 p-3"><p className="text-xs text-muted-foreground">Fines currently applied</p><p className="mt-1 font-bold text-destructive">{money(rows.reduce((sum, row) => sum + Number(row.penaltyAmount || 0), 0), currency)}</p></div></div><div className="overflow-x-auto rounded-lg border"><Table><TableHeader><TableRow><TableHead>#</TableHead><TableHead>Due date</TableHead><TableHead className="text-right">Principal</TableHead><TableHead className="text-right">Interest</TableHead><TableHead className="text-right">Expected late fine</TableHead><TableHead className="text-right">Paid</TableHead><TableHead className="text-right">Balance</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{rows.map(row => <TableRow key={row.id}><TableCell>{row.installmentNumber}</TableCell><TableCell>{row.dueDate}</TableCell><TableCell className="text-right">{money(row.principalAmount, currency)}</TableCell><TableCell className="text-right">{money(row.interestAmount, currency)}</TableCell><TableCell className="text-right"><span className={row.penaltyAmount > 0 ? 'font-bold text-destructive' : 'text-amber-700'}>{money(row.penaltyAmount > 0 ? row.penaltyAmount : loan.latePaymentFine || 0, currency)}</span><span className="block text-[10px] text-muted-foreground">{row.penaltyAmount > 0 ? 'Applied' : 'If missed'}</span></TableCell><TableCell className="text-right">{money(row.paidAmount, currency)}</TableCell><TableCell className="text-right font-semibold">{money(row.balance, currency)}</TableCell><TableCell><Badge variant={row.status === 'PAID' ? 'default' : row.status === 'OVERDUE' ? 'destructive' : 'secondary'}>{row.status}</Badge></TableCell></TableRow>)}</TableBody></Table></div></> : <p className="text-sm text-muted-foreground">Repayment dates appear after the final approval and disbursement.</p>}</CardContent></Card>
    </>}
  </main>
}
