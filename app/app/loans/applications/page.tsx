...
import { useLoans, type Loan, type LoanRepayment } from '@/hooks/useLoans'
...
export default function LoanApplicationsPage() {
  ...
  const [repaymentDecision, setRepaymentDecision] = useState<LoanRepayment | null>(null)
  ...
  const refresh = async (id: string) => {
    if (!id) return
    setLoading(true)
    try { const [loanRows, repaymentRows] = await Promise.all([api.list(id), api.repayments(id)]); setLoans(loanRows); setRepayments(repaymentRows) }
  ...
  const reviewRepayment = async (repayment: LoanRepayment, approve: boolean) => {
    if (actingId !== null) return
    setActingId(repayment.id)
    try {
      if (approve) await api.approveRepayment(groupId, repayment.id)
      else await api.rejectRepayment(groupId, repayment.id, reason.trim())
      toast.success(approve ? 'Repayment approved and allocated across the repayment schedule.' : 'Repayment rejected.')
      setRepaymentDecision(null)
      setReason('')
      await refresh(groupId)
      window.dispatchEvent(new Event('vikoba:approval-updated'))
    } catch (error) { toast.error(error instanceof Error ? error.message : 'Unable to review repayment') }
    finally { setActingId(null) }
  }
  const pendingRepayments = repayments.filter(item => item.status === 'PENDING')
  ...
  return <main className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
    ...
    {pendingRepayments.length > 0 && <Card><CardHeader><CardTitle>Loan repayments awaiting verification</CardTitle><p className="text-sm text-muted-foreground">Only the assigned accountant can confirm that money was received. Approval allocates overpayments into the next installments automatically.</p></CardHeader><CardContent><div className="overflow-x-auto rounded-lg border"><Table><TableHeader><TableRow><TableHead>Submitted</TableHead><TableHead>Member / loan</TableHead><TableHead>Amount</TableHead><TableHead>Method</TableHead><TableHead>Reference</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>{pendingRepayments.map(item => <TableRow key={item.id}><TableCell>{dateTime(item.submittedAt)}</TableCell><TableCell><p className="font-medium">{item.memberName}</p><p className="text-xs text-muted-foreground">{item.loanNumber}</p></TableCell><TableCell className="font-semibold">{money(item.amount)}</TableCell><TableCell>{item.paymentMethod.replaceAll('_', ' ')}</TableCell><TableCell>{item.externalReference || '—'}</TableCell><TableCell><div className="flex justify-end gap-2">{item.canApprove ? <><Button size="sm" disabled={actingId !== null} onClick={() => void reviewRepayment(item, true)}>{actingId === item.id ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />} Approve payment</Button><Button size="sm" variant="destructive" disabled={actingId !== null} onClick={() => { setRepaymentDecision(item); setReason('') }}>Reject</Button></> : <span className="text-xs text-muted-foreground">Waiting for accountant</span>}</div></TableCell></TableRow>)}</TableBody></Table></div></CardContent></Card>}
    ...
    <Dialog open={!!repaymentDecision} onOpenChange={open => { if (!open && actingId === null) setRepaymentDecision(null) }}><DialogContent><DialogHeader><DialogTitle>Reject repayment?</DialogTitle><DialogDescription>The member will see this reason and can submit the correct payment again.</DialogDescription></DialogHeader><div className="space-y-2"><Label htmlFor="repayment-rejection-reason">Reason</Label><Textarea id="repayment-rejection-reason" value={reason} onChange={event => setReason(event.target.value)} placeholder="Explain why the payment could not be verified" /></div><DialogFooter><Button variant="outline" disabled={actingId !== null} onClick={() => setRepaymentDecision(null)}>Cancel</Button><Button variant="destructive" disabled={!reason.trim() || actingId !== null} onClick={() => { if (repaymentDecision) void reviewRepayment(repaymentDecision, false) }}>{actingId !== null && <Loader2 className="size-4 animate-spin" />} Reject repayment</Button></DialogFooter></DialogContent></Dialog>
