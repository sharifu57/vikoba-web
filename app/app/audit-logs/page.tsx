    ...
    return (
        <main className="mx-auto max-w-7xl px-6 py-8">
            ...
            <section className="mb-6 grid gap-4 md:grid-cols-[1fr_220px_auto]">
                ...
                <label className="flex items-center gap-2 rounded-xl border border-[#E5E7EB] bg-white px-4 text-xs font-bold text-neutral-600 shadow-sm"><Filter size={14} className="text-[#0B6B50]" /><select value={action} onChange={(event) => setAction(event.target.value)} className="w-full bg-transparent py-3 outline-none"><option value="ALL">All actions</option>{actions.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
